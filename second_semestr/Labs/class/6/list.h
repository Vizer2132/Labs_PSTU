...
public:
	...
	List() {};
	List(initializer_list<T> init) { // дл€ записи list = { 1, 2, 3 }
		for (const T& value : init) {
			push_back(value);
		}
	}
	List(const List& newList) : size(0), firstElementAddress(nullptr), endElementAddress(nullptr) {
		//					  |_________________________дл€_newList______________________________|
	...
	friend List<T> operator*(const T& value, const List<T>& list) {
		return list * value;  // симметрична€ верси€
	}
